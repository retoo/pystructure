...
This leads to the question, how can the type of a given expression be determined? In Java this question would be very simple, the definition always states the type.\footnote{In the example \java{boolean isOdd(int i)}, method \java{isOdd} has a parameter \java{i} with the type \type{int} and returns a \type{boolean}.} In Python the types aren't specified at all, they are only known at runtime by the interpreter. To still be able to determine them, a heuristic has to be used which infers the types by looking at the context and finding answers to questions like ``Where was the variable defined?'' and ``Who called that function?''. This heuristic is usually called type inferencer (TI).
...
\begin{itemize}
	\item The Type inferencer gets initialised with a \class{ExpressionTypeGoal} with the expression \node{Name(x)} attached to it.
	\item The evaluator factory assesses the goal and assigns it to the \class{NameTypeEvaluator} because the expression in question is a Name AST Node.
	\item The \class{NameTypeEvaluator} finds the definition of \id{x} by looking for the corresponding \class{NameUse}. It then creates a \class{DefinitionTypeGoal} to determine it's type. Note: if there is more than one definition the evaluator just creates a goal for each of them. \todo{we could mention somewhere that this easy parallelising a advantage of the goal engine is}
	\item In this case the node attached to the definition is a \node{Assign} node and therefore the goal gets mapped to the \class{AssignTypeEvaluator}.
	\item All the \class{AssignTypeEvaluator} does is creating a new \class{ExpressionTypeGoal} for the assigned value. In this case this is: \code{Foo()}.
	\item Note: As \id{Foo} is capitalised it is quite obvious that we are talking about the instantiation of an object here. But Python allows capitalised functions and therefore the TI can't be completely sure and therefore has to check what the type returned by that call is.
	\item This new \class{ExpressionTypeGoal} gets also processed by the evaluator factory, but this time the goal gets assigned to the \class{CallTypeEvaluator} as the goal's node is a call: \code{Foo()}.
	\item The evaluator first tries to determine if the name in question is a function or a class name. To do that it creates a expression type goal for the name. In this case this is the name \id{Foo}.
	\item Now the process begins from the start, the type of the name \id{Foo} has to be evaluated. The \class{NameTypeEvaluator} finds the class definition, and the results in a \class{MetaClassType} being returned.
	\item The result gets handed back to the \class{CallTypeEvaluator} which now knows that the call is in fact made to a constructor, so therefore the type returned by the call is a object of the class \class{Foo}. 
...
\begin{itemize}
	\item The first few steps are similar to the first example, the evaluator eventually creates a \class{ExpressionTypeGoal} for the expression \code{function()}.
	\item Because it is a \node{Call} node as above the \class{CallTypeEvaluator} gets involve again. And as in the last example the name of the function being called is resolved: \id{function}
	\item The name \id{function} leads to the function definition and gets returned to the \class{CallTypeEvaluator}.
	\item Now knowing that this is a function the evaluator creates a \class{ReturnTypeGoal} for it.
	\item \class{ReturnTypeGoal}'s are handled by the \class{ReturnTypeEvaluator}, which does not much more than creating a \class{ExpressionTypeGoal} for every return statement in the function. 
...
The next example shows a different important problem: How are the types of arguments being found. So the TI doesn't have to find the function, but the other way around: all callers of a function.\todo{rschuett: write me differently}
...
\begin{itemize}
	\item Again the beginning is quite similar to the previous examples, \id{x} gets assigned from \id{arg}. And \id{arg} is defined in a \node{Argument} node. The \class{DefinitionTypeGoal} of it gets therefore assigned to the \class{ArgumentTypeEvaluator}.
	...
	\item The \class{PossibleReferencesEvaluator} iterates over all name uses (in all modules) and checks if their name match with the function being looked for. \textbf{} \\\textbf{Note:} \emph{The \class{PossibleReferencesEvaluator} doesn't really care if the name uses found are actually calls or just something else which just happens to be named the same way.}
	\item To find out which one of these potential references are real callers the  \class{FunctionReferencesEvaluator} iterates over all them and creates for each one a \class{ExpressionTypeGoal}. 
	\item The expressions get resolved and the \class{FunctionReferencesEvaluator} just keep the references which point back to the function in question.
	\item Finally with all the callers known the \class{ArgumentTypeEvaluator} can evaluate the types of the passed arguments by creating \class{ExpressionTypeGoals} for each one.
...
In PyStructure, the constructor call node is used to differentiate between instances of the same class. For this, an additional field \java{constructorCall} was added to \class{ClassType}, the object representing an instance of a class. It is set when a \class{ClassType} is created.
...
%{ rschuett: korrigieren, vielleicht ein bisschen mehr blabla :)
...
Instead of just adding debugging code to the goal engine a logging interface was defined which then gets called by the engine during the different stages of the evaluation. The interface \class{IGoalEngineLogger} describes a set of hooks which can be used for a wide variety of debuggers and loggers: The beginning and the end of the evaluation process (\java{evaluationStarted()} \& \java{evaluationFinished()}) and two hooks which get called when a goal has been created or finished (\java{goalCreated()} \& \java{goalFinished)}).
