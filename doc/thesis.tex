...
The \code{##} is the start marker for an assertion, the \code{type} is the name of the assertion and the part after that is the expected value. For each assertion, a goal is created and passed to the goal engine. The result is stringified and compared with the expected value. If it differs, an \type{InferencerAssertionError} is thrown with the information about the failure, for example:
