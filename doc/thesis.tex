...
When developing software it is useful to get an overview of the source code of a project, for example when one is new to a project or wants to reorganise its structure. When directly looking at the source code, it is difficult to get a high-level perspective of the components and their dependencies. Graphical models commonly used like domain or architectural models (UML) help, but they usually  describe the \emph{intended} design, rather than the \emph{actual} structure of a program.
% This is when a dedicated structure analysis program is useful.
% In such cases it is often useful to do a structural analysis of the application.

This project's goal was to develop a structural analyser for programs which are written in the Python programming language. The analyser should be able to parse an application's source code, analyse it and then generate a graph representing the internal structure of the project.

As Python is a dynamic language most of the interesting details (i.e. types) are not known before the application is running. The analyser has to ``guess'' the correct types by looking at the whole code base. This is done with a heuristic called a type inferencer.

The analyser provides the following information:
...
The generated data can be used by other programs for various further analysis. One example is Structure 101g developed by Headway Software which takes arbitrary dependency data and visualises it. But the data is also useful for various software development topics like code completion or automated refactorings in IDEs.
