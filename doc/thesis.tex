...
% rstocker { reread
Python modules are represented as \emph{module} elements. The module's name and its full path within the project folder are specified in the \emph{name} attribute. The packages itself are not specified, they have to be implicitly recognised out of the used module path. E.g. when a module \emph{drivers.sound} is exported, it is implicitly assumed that there is a package \emph{drivers}.
