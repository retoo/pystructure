...
\todo{rstocker: bah, ich finde das cycles zeugs ned so gut}
...
The \class{PythonEvaluatorFactory} takes several criteria into account. In the case of an expression type goal the type of the associated expression is also considered. For example if the expression is a method call the goal is delegated to the \class{CallTypeEvaluator}, an attribute expression is processed by the \class{AttributeTypeEvaluator} and so on.\todo{rstocker: Should we a provide a list of these mappings somewhere?}
...
\begin{itemize}
    ...
    \item The \class{AttributeTypeEvaluator} is responsible for evaluating \node{Attribute} nodes
    ...
    \item The \class{MethodResolveGoal} is handled by the \class{MethodResolveEvaluator} which checks if there's a method with a given name in a particular class or base classes.\\
          In the example \id{cargo} is a attribute and therefore a method can't be found
    \item Because the \class{MethodResolveGoal} was unsuccessful \class{AttributeTypeEvaluator} evaluates if there is a attribute with this name by issuing a \class{ClassAttributeTypeGoal}.
    ...
    \item The \class{AttributeReferencesEvaluator} works similar to the \class{FunctionReferencesEvaluator} described in the second example:
    \begin{enumerate}
        \item Find all potential references uses using the \class{PossibleReferencesEvaluator}
...
Using the information gathered by the type inferencer it is now possible to automatically detect the internal structure of a application. The structure consists of the different components of the application, its packages, modules and classes and the dependencies between them. Expressed as a directed graph the different components would be the vertices, where as the dependencies are edges. 
...
But how are these information extracted out of the application? There is the type inferencer which is able to determine the type of a single expression, but how does    find all
