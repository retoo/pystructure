...
%{ rschuett: Korrekturen anschauen
...
The types for \id{a} and \id{b} should be the same as in the example with the lists. But with a type inferencer not aware of different instances, the result is actually \type{int|str} for both variables. Why is that? Looking at the evaluation for the type of \id{a}, the problem becomes clear (the finding of references was ommited for brevity):
