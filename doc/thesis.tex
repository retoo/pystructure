...
Some details were removed to enhance readability. This example also illustrates the fact that goals don't have to be created or finished in a strict order, sometimes a different goal is processed before a finished one is cleaned up.
...
The structure is exported for further external analysis as an XML file. The content is separated into two parts, first a list of the components and second as the list of dependencies between the components.

Python modules are represented as \emph{module} elements. The modules name and its full path within the project folder are specified in the \emph{name} attribute. All components beneath modules are stored using the \emph{submodule} tag, with an attribute \emph{type} specifying the actual type of component (\emph{class, function, method or attribute}) and their name in the \emph{name} attribute.

All modules and sub modules are given an unique identifier in the attribute \emph{id}. PyStructure uses the full qualified module name (in Python module notation) for the modules and the Java hash value for all other components. These identifiers are used to define the dependencies. 
...
The second part of the XML provides a list of all dependencies between the components. This is simply a list of \id{dependencies} elements, each of them specifying a \emph{type} (\emph{calls, references, extends, parameter and is type}) and a source and target component using the attributes \emph{from} and \emph{to}.

It should be noted \todo{darf man das so sagen?} that only the actual explicit dependencies are stored. The inherited dependencies between the high level components are omitted.
