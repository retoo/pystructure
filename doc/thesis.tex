...
% Zu lang: When looking directly at source code, either because one is new to it or one wants to refactor some parts, it is difficult to get a high-level perspective of the components and their dependencies. ...

% Von Proposal: Planning and analysing a program's structure and internal dependencies is a common task when working on larger projects. Such analysis helps to meet the defined architectural design goals or to find structural flaws like cyclic dependencies. A clear architecture becomes more and more important as software grows.

A vital part of software development is to define a good architectural model when planning a project. But during implementation the intended structures are often neglected and the program's architecture grows in a different way than planned – a problem called \emph{architectural decay}. When only looking at the source code,  tracking the architecture and keeping a high-level view of the dependencies between components is difficult. Commonly used graphical models such as domain or architecture diagrams are helpful, but they usually just describe the \emph{intended} design, rather than the \emph{actual} structure.

PyStructure's goal was to develop an analyser which reveals these internal structures and dependencies in a Python program. The necessary information has to be gathered by statically analysing its source code. Dependencies are identified by determining what types are used in a particular component. But as Python is a dynamically typed language the types are not known before the application is run. Therefore a major aspect of the project was to develop an algorithm, called type inferencer, which is able to deduce types by mimicking the evaluation process of the interpreter.
...
The extracted structure can then be visualised by Structure 101g, a program developed by Headway Software. Structure 101g presents the dependencies as graphs in various levels of detail or abstraction. It also provides a wide variety of analysis tools, like detecting cyclic dependencies or validating the current structure against a predefined architecture. This allows the user to effectively monitor the progress of a project's architecture during its development.
...
Therefore PyStructure uses a second stage in which it processes the AST further. It does some static analysis to create a model which provides a base for the type inferencer to work with. The model consists of the following:
...
% { rstocker: weiter betoneen, zu fiel
At this stage types are still ignored, they are solely handled in the third stage described further below.
%}
...
Now we have the elements of the model, but to make the type inference step easier, more is needed. As described in the example with the \id{foo} variable, knowing which variable definitions and uses are related would be very useful. This is part of what the \class{DefinitionVisitor} does and is called flow analysis.

Flow analysis is an important part of type inference. In Python, it is possible that a variable is assigned multiple times with different types. The type inferencer can still determine the type uniquely if the values are assigned sequentially and without any \code{if} blocks:
...
\todo{rstocker : How should we call the user of the library? just user? or caller? we could mention somewhere in the beginning that by saying ``user'' we mean ``user of the library''}
...
\todo{rstocker/rschuett: somehwere in the TI chapter explain what is meant by goals and why these are stored in classes}
...
\begin{itemize}
	...
	\item The TI (the evaluator factory) assesses the goal and assigns it to the \evalautor{VariableReferenceEvaluator} because the expression in question is a Name AST Node
\end{itemize}
...
Without separating these two uses, the type inferencer would answer that the type of both \id{a} and \id{b} is \type{int|float}. But a more precise answer would be that \id{a} is \type{int} and \id{b} is \type{float}. It is necessary to analyse return types differently depending on how the functions are called, it is necessary to respect the \emph{call context}.

Let's look at the evaluation of the return type of the first \id{add_one} call to explain how it is solved in PyStructure. The information about the call is stored in a \class{CallContext}. It is created in the \class{CallTypeEvaluator} for function calls (as opposed to constructor calls, which don't need a call context). The call context contains the following information:
...
The \class{CallTypeEvaluator} then creates a \class{ReturnTypeGoal} and associates the previously created context with it. The evaluation then proceeds as usual.

If the return value doesn't depend on an argument type, the call context will never be needed, which is fine. But when it does and the evaluation comes to an argument, the \class{ArgumentTypeEvaluator} is run. In the example this would be for the \id{value} argument of \id{add_one}. The evaluator then gets the ``topmost'' call context and determines whether the call context applies by comparing the function of the argument it has to evaluate and the function of the call context. If they are equal, this means that the call context is actually for this function and not for another one.

So, if the call context applies, the expression which corresponds to the argument is determined and a subgoal for the type of the expression is created. In the example, the expression is \code{41} and its type will be \type{int}. This type is then returned as the result of the \class{ArgumentTypeEvaluator}, leading to the final result of \type{int} for the \class{CallTypeEvaluator}.

If the call context doesn't apply, the \class{ArgumentTypeEvaluator} does the usual thing of finding all the calls and returning the union of all the argument expressions.
