...
When developing software it is important to have an overview of a program's internal structure. For example when getting acquainted with a new project or when a reorganisation of its structure is necessary. When directly looking at the source code, it is often difficult to get a high-level view of the components and the dependencies between them. Graphical models commonly used such as domain or architecture diagrams help, but they usually just describe the \emph{intended} design, rather than the \emph{actual} structure of a program. 
