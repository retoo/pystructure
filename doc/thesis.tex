\documentclass[12pt,halfparskip,BCOR10mm]{scrreprt}
...
\begin{itemize}
	...
	\item The class hierarchy can't be statically resolved during the creation of the model, because the specification of the base class is just an expression and could be the result of a function call or an imported class}\footnote{
	For example: \code{class Foo(method()): pass}. Of course this kind of base class definition is not seen that often.}.
	% TODO: do a reference to the MRO C3 section
	\item What exactly is being imported by an import statement.
	% r schuett: Hast du noch mehr Ideen? s
	% rstocker: jap, nämlich das es im model immer noch mehrer wege gibt um das selbe auszudrücken: bsp. __add__() und +(). Also quasi ast ist halt immer noch ast
\end{itemize}
