...
Let's look at the evaluation of the return type of the first \id{add_one} call to explain how it is solved in PyStructure. The information about the call itself is stored in a \class{CallContext}. It is created in the \class{CallTypeEvaluator} for function calls. The call context contains the following information:
...
The \class{CallTypeEvaluator} then creates a \class{ReturnTypeGoal} of the \id{add_one} function and associates the previously created context with it. The evaluation then proceeds as usual. 

Now two situations are possible:

\textbf{Argument doesn't affect return value}

This means that the evaluation works as usual, meaning that the evaluator has to look for all possible callers and check what they all pass as an argument. 

\textbf{Argument affects return value}

The case happens if during the evaluation of the return type the  \class{ArgumentTypeEvaluator} is being run. In the example above this would happen when the evaluator tries to find the type of the argument \id{value}. The trick is now the check if only the call in the context (i.e. the \class{CallContext}) should be considered. To check that the evaluator verifies that the function in the ``topmost'' call context points to the function currently being evaluated. 

If this is the case the evaluator doesn't evaluate the parameter of all callers, but only the one mentioned in the call context. In the example this would mean that only the first call \code{add_one(41)} is taken into consideration.

If the call context doesn't point to the current function, the \class{ArgumentTypeEvaluator} does the usual thing of finding all the calls and returning the union of all the argument expressions.
