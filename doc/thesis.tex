...
% rschuett: subsection korrigieren
...
Flow analysis is an important part of type inference. In Python, it is possible that a variable is assigned multiple times with different types. This doesn't matter if the values are assigned sequentially and without conditional branches\todo{was heisst das?}:
...
The two definitions are made unconditionally, which means the two uses of var each belong to the definition right before them. So, the type of the first \id{var} is \type{int}, while the second's is \type{str}.

If an analyser doesn't actually respect the flow of the variables and just takes all assignments to the same name as belonging to the same variable, it is called flow-insensitive. The type of all \id{var} names in the example then would be \type{int|str}. PyStructure is flow-sensitive, which means it is as precise as possible when there's more than one assignment of a variable in the same scope. The advantage is that it results in better precision for inferring types and it is also a very useful feature to have for the use case of an IDE, where it helps with certain refactorings.


\todo{ich wuerde hier noch erwähnen dass diese analyse unabhängig ovn den eigentlichen typen ist. ALso z.b. wenn da ned var=42 sondern var = some func() stehen wuerde koennte die benutzung von var trotzdem schon statisch an die definition gehaengt werden, obwohl der typ ja nicht bekannt ist}
...
For example in the case of a expression type goal the evaluator factory also considers the type of the enclosing expression. Is it a method call the 

\subsubsection{Evaluator}
Now the actual work begins, the evaluator gets initialised with the goal as a parameter. THe 
...
% rschuett: Einführung korrigieren

In this section, some of the difficult parts of developing a type inference system for Python are inspected. Each problem is illustrated with examples and then the solution which was implemented in PyStructure is explained.
...
% rschuett: Jetzt ist Beschreibung des Problems gleich nach erstem Beispiel, gut?
